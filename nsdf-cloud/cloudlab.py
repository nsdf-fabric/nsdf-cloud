...
class CloudLabEc2(BaseEc2):
	...
	def __init__(self, config):
		...
		self.profile_name=self.config["profile-name"]		

		logging.info(f"\t [project_name={self.project_name}]")
		logging.info(f"\t [profile_name={self.profile_name}]")	 
	...
	def createNodes(self, args):
		...
		conf={
			...
			"name"	: f"{uid}",
			'bindings' : """{"nodeCount" : "$num" }""".replace("$num",str(self.num))
		}
