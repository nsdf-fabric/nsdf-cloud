...
class ChameleonEC2(BaseEc2):
	...
	def createNodes(self, args):
		...
		if True:
			...
			reservation_id = get_node_reservation(uid)
		...
		if True:
			for I in range(self.num):
				...
				logging.info(f"Waiting for server {name} {I}/{self.num}...")
				server_id = get_server_id(name)
				logging.info(f"Got id {server_id}")
				logging.info(f"wait for server (NOTE: can take 30 minutes)")
